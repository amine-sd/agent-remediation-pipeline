...
def ingest_day(day: date, raw_dir: Path = RAW_DIR,
               fetch: Callable[[date], bytes] = fetch_archive) -> dict:
    """Land one day and return a short summary. Raises if the source fails.

    `fetch` is a parameter so that tests, and later the fault injector, can replace the
    network call without touching this function.
    """
    ...
    _write_csv(tmp / "stations.csv", STATION_COLUMNS, stations)
    _write_csv(tmp / "prices.csv", PRICE_COLUMNS, prices)
    tmp.rename(target)
    return {"date": day.isoformat(), "status": "ingested",
            "stations": len(stations), "prices": len(prices)}
