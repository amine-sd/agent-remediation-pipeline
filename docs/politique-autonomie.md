...
| Escalader | Ouvre un ticket avec son diagnostic | Escalader |
...
### Toujours : les outils de lecture
...
C'est le moyen d'escalader. Le ticket contient la ou les causes identifiées, la justification, et
ce que l'agent aurait proposé de faire.

### La liste blanche : une seule action

**Relancer l'étape d'ingestion, au plus une fois par incident.**
...
L'outil « relancer une étape » accepte n'importe quelle étape, mais le garde-fou n'exécute que
l'ingestion, et une seule fois par incident. Toute autre demande est refusée, tracée, et
transformée en ticket. Garder l'outil général et restreindre au niveau du garde-fou permet de
vérifier, par un test, que le refus fonctionne vraiment.
...
1. **Une relance a déjà eu lieu** pour cet incident, et la panne persiste. L'agent le sait par les
   journaux : chaque relance y est inscrite par le garde-fou.
...
Chaque décision est journalisée : horodatage, exécution concernée, cause ou causes, décision,
justification, action demandée, et ce que le garde-fou en a fait (exécutée ou refusée).
